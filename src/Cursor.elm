...
fromList list =
    case list of
        ...
        [] -> Nothing
        --list has elements, build cursor
...
toList (Cursor left mid right) =
    List.concat [List.reverse left, [mid], right]
...
forward (Cursor left mid right) =
    case right of
        [] -> Nothing
        x :: xs -> Just (Cursor (List.concat [[mid], left]) x xs)
...
back (Cursor left mid right) =
    case left of
        [] -> Nothing
        newMid :: newLeft -> Just (Cursor newLeft newMid (List.concat [[mid], right]))
